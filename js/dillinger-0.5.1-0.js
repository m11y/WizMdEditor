...
$(function(){
  ...
  function asyncLoad(filename,cb){
    (function(d,t){
      ...
      leScript.async = 1
      leScript.src = wizMdPluginPath + filename;

      var done = false;
      leScript.onload =  leScript.onreadystatechange = function() {
  ...
  function updateUserProfile(obj){
    var str = JSON.stringify( $.extend(true, profile, obj) );
    //alert('store profile:' + str);
  }
  ...
  function initUi(){
    ...
    fetchTheme(profile.theme, function(){
      ...
      editor.getSession().setValue(initContent);
      //editor.getSession().getDocument().insertLines(0,initContent);
